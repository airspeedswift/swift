...
/// If \p inst is a lifetime-ending use that consumes the payload of the opaque
/// existential marked by \p markedValue, projected out via an
/// open_existential_addr, return the existential container address. Its backing
/// storage (which may be an out-of-line box) must be deallocated with
/// deinit_existential_addr after the consume: the payload has been moved out,
/// so a destroy_addr would double-destroy it and doing nothing leaks the box.
///
/// Returns an invalid SILValue if this use is not such a consume.
static SILValue
getConsumedOpenedExistentialContainer(SILInstruction *inst,
                                      SILValue markedValue) {
  // The opened payload must be rooted at the existential we're processing, so
  // that we deinit exactly the container whose payload this use consumed (and
  // not some other existential opened by the same instruction).
  auto containerOf = [&](SILValue operand) -> SILValue {
    auto *oeai =
        dyn_cast<OpenExistentialAddrInst>(stripAccessMarkers(operand));
    if (!oeai)
      return SILValue();
    if (!oeai->getOperand()->getType().canUseExistentialRepresentation(
            ExistentialRepresentation::Opaque))
      return SILValue();
    if (stripAccessMarkers(oeai->getOperand()) != markedValue)
      return SILValue();
    return oeai->getOperand();
  };

  // A copy_addr take of the opened payload (the move-only checker turns the
  // copy into a take when it is a lifetime-ending use).
  if (auto *copy = dyn_cast<CopyAddrInst>(inst))
    return containerOf(copy->getSrc());

  // An @in (owned, indirect) argument that is the opened payload.
  if (auto apply = FullApplySite::isa(inst)) {
    for (auto &op : apply.getArgumentOperands()) {
      if (!apply.getArgumentConvention(op).isOwnedConventionInCaller())
        continue;
      if (SILValue container = containerOf(op.get()))
        return container;
    }
  }
  ...
  for (auto &pair : boundary.getLastUsers()) {
    ...
    for (auto rangePair : ranges) {
      ...
      case IsInterestingUser::LifetimeEndingUse: {
        ...
        //
        // rdar://163574532: if this use consumes the payload of an opaque
        // existential through an open_existential_addr, the existential's
        // backing storage (which may be an out-of-line box) must still be
        // deallocated -- the payload has been moved out, so emit a
        // deinit_existential_addr after the consume to free the box without
        // re-destroying the moved-out payload. (On paths where the existential
        // is *not* consumed it is torn down normally with destroy_addr below.)
        SILValue existentialToDeinit =
            getConsumedOpenedExistentialContainer(inst, markedValue);
        if (auto *ti = dyn_cast<TermInst>(inst)) {
          for (auto *succBlock : ti->getSuccessorBlocks()) {
            insertUndefDebugValue(&succBlock->front());
            if (existentialToDeinit) {
              SILBuilderWithScope deinitBuilder(&succBlock->front());
              deinitBuilder.createDeinitExistentialAddr(
                  RegularLocation::getAutoGeneratedLocation(inst->getLoc()),
                  existentialToDeinit);
            }
          }
        } else {
          ...
          if (existentialToDeinit) {
            SILBuilderWithScope deinitBuilder(inst->getNextInstruction());
            deinitBuilder.createDeinitExistentialAddr(
                RegularLocation::getAutoGeneratedLocation(inst->getLoc()),
                existentialToDeinit);
          }
