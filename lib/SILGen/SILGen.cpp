...
/// SIL requires a function to have a single return block, but each 'become'
/// emits its guaranteed tail call immediately followed by its own 'return', so
/// that nothing can come between the two. Restore the invariant by routing
/// every return through one shared epilog block. IRGen re-creates the adjacency
/// the tail call needs by emitting the 'ret' at the musttail call's terminator.
static void mergeBecomeReturnBlocks(SILFunction &F) {
  SmallVector<ReturnInst *, 4> returns;
  bool hasMustTailCall = false;
  for (auto &bb : F) {
    for (auto &inst : bb) {
      if (auto *ai = dyn_cast<ApplyInst>(&inst))
        hasMustTailCall |= ai->isMustTailCall();
      else if (auto *tai = dyn_cast<TryApplyInst>(&inst))
        hasMustTailCall |= tai->isMustTailCall();
    }
    auto *term = bb.getTerminator();
    if (auto *ret = dyn_cast<ReturnInst>(term))
      returns.push_back(ret);
    else if (isa<ReturnBorrowInst>(term))
      // A borrowed return has its own epilog shape; leave the function alone.
      return;
  }
  if (!hasMustTailCall || returns.size() < 2)
    return;

  auto resultTy = F.getConventions().getSILResultType(
      F.getTypeExpansionContext());
  auto ownership = resultTy.isTrivial(F) ? OwnershipKind::None
                                         : OwnershipKind::Owned;
  auto *epilogBB = F.createBasicBlock();
  auto *result = epilogBB->createPhiArgument(resultTy, ownership);
  auto loc = RegularLocation::getAutoGeneratedLocation();
  SILBuilder(epilogBB, F.getDebugScope()).createReturn(loc, result);

  for (auto *ret : returns) {
    SILBuilderWithScope builder(ret);
    builder.createBranch(ret->getLoc(), epilogBB, {ret->getOperand()});
    ret->eraseFromParent();
  }
}
