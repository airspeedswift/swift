...
class PatternMatchEmission {
  ...
  /// Set when we've emitted the
  /// `noncopyable_shared_case_block_unimplemented` diagnostic for this
  /// pattern-match. Both `emitSwitchFallthrough` (when it hits a noncopyable
  /// address-typed binding it can't pass through a phi) and
  /// `emitSharedCaseBlocks` may try to emit it; this flag ensures the user
  /// sees it exactly once.
  bool noncopyableSharedCaseDiagnosticEmitted = false;
...
public:
  ...
  /// Emit `diag::noncopyable_shared_case_block_unimplemented` once, attached
  /// to `anyCase`. Idempotent across multiple invocations on the same
  /// emission. Called from `emitSwitchFallthrough` (when a noncopyable
  /// address-typed binding can't be passed through a shared-case phi) and
  /// from `emitSharedCaseBlocks` (the existing fallback for non-fallthrough
  /// shared cases).
  void emitNoncopyableSharedCaseUnimplementedDiagnostic(CaseStmt *anyCase);
...
void PatternMatchEmission::bindBorrow(Pattern *pattern, VarDecl *var,
  ...
  if (bindValue.getType().isObject()) {
    if (sourceIsNoncopyable) {
      ...
      //
      // The alloc_stack and store_borrow are pure compiler artifacts (the
      // user wrote a `let` binding, not a stack spill), so mark their
      // locations as auto-generated to keep them out of debug stepping.
      SILLocation tmpLoc = pattern;
      tmpLoc.markAutoGenerated();
      auto temp = SGF.emitTemporaryAllocation(tmpLoc, bindValue.getType());
      bindValue = SGF.B.createStoreBorrow(tmpLoc, bindValue, temp);
    } else {
...
void PatternMatchEmission::emitNoncopyableSharedCaseUnimplementedDiagnostic(
    CaseStmt *anyCase) {
  if (noncopyableSharedCaseDiagnosticEmitted)
    return;
  noncopyableSharedCaseDiagnosticEmitted = true;
  SGF.SGM.diagnose(anyCase, diag::noncopyable_shared_case_block_unimplemented);
}
...
void PatternMatchEmission::emitSharedCaseBlocks(
  ...
  if (ownership >= ValueOwnership::Shared
      && !SharedCases.empty()) {
    emitNoncopyableSharedCaseUnimplementedDiagnostic(
        SharedCases.front().first);
  ...
  for (auto *expected : destCaseStmt->getCaseBodyVariables()) {
    ...
    for (auto *var : fallthroughSourceStmt->getCaseBodyVariables()) {
      ...
      if (value->getType().isAddress() && value->getType().isMoveOnly()) {
        ...
        // for a noncopyable type. Multi-pattern noncopyable cases are not
        // yet implemented; emit the diagnostic directly here so that the
        // unreachable below has a clear, tested cause (deduped against any
        // later emission from `emitSharedCaseBlocks`).
        //
        // We intentionally `return` after `createUnreachable` rather than
        // falling through to `emitBranchAndCleanups` at the end of this
        // function. That call only emits *code* for cleanups (with
        // popCleanups=false) and a branch to the shared-case block — both
        // unreachable here, so omitting them is safe. The cleanup stack
        // itself is untouched and the enclosing scope will pop it normally.
        context->Emission
            .emitNoncopyableSharedCaseUnimplementedDiagnostic(destCaseStmt);
        B.createUnreachable(S);
        return;
      }
