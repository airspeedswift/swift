...
extension RangeExpression where Self: Collection, Self.Index == Self.Element {
  ...
  @inline(__always)
  public func overlaps<R: RangeExpression>(_ other: R) -> Bool
  where R.Bound == Bound, 
        R: Collection, R.Element == Self.Element, R.Index == R.Element {
    return (!other.isEmpty && self.contains(other.startIndex))
        || (!self.isEmpty && other.contains(endIndex))
  }
}
